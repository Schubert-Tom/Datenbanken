...
$(document).ready(function () {
  ...
  $("#btn_new_User").click(function (event) {
    ...
    if (
      participants.indexOf(user_to_verify) >= 0 ||
      ...
      .then(function (response) {
        ...
        response.json().then(function (data) {
          ...
          // Wenn User  valid, dann neues Textfeld eröffnen und User hinzufügen
            ...
            $("label[for='participants']").before(
              "<label class='js_made'>Added User: " +
                data["user_to_verify"] +
                "</label><br class='js_made'>"
            );
            // Textfeld leeren
            $("#participants").val("");
  ...
  $("#CreateChat").submit(function (event) {
    ...
    if (chat_name.length < 2 || chat_name.length > 15) {
      ...
      $(".error_add_title").val(
        "Der Chat-Name darf mindestens 2 und maximal 15 zeichen beinhalten"
      );
      return;
    }
